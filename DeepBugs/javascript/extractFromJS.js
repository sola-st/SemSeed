...
`;
    ...
    if (args[1] === "--parallel") {
        ...
        if (what === "binOps") extractor = require("./extractorOfBinOps");
        ...
        let outDir = path.join('./benchmarks', what);
        if (!fs.existsSync(outDir))
            fs.mkdirSync(outDir)
        let fileName = what + "_" + Date.now() + ".json";
        if (outDir)
        ...
        let extractor;
        if (what === "binOps") extractor = require("./extractorOfBinOps");
        if (what === "assignments") extractor = require("./extractorOfAssignments2");
        let location = args[3];
        if (location === 'null') {
            location = null;
        }
        const allData = [];
        const jsFile = args.slice(2)[0];

        // let fileToID = getOrCreateFileToID([jsFile]);
        if (what !== "tokens") {
            // const fileID = fileToID[jsFile]; // assumption: at most 6 digits long
            visitFile(jsFile, extractor, allData, location);
        } else {
            // extractor.visitFile(jsFile, allData);
        }
        if (allData.length === 0) {
        ...
        if (!fs.existsSync(outDir))
            fs.mkdirSync(outDir)
        let fileName = what + "_" + Date.now() + "_" + Math.floor(Math.random() * 100000) + '_' + path.parse(jsFile).name + ".json";
        if (outDir)
            fileName = path.join(outDir, fileName);
        console.log("Writing " + allData.length + " items to file " + fileName);
        fs.writeFileSync(fileName, JSON.stringify(allData, null, 2));
