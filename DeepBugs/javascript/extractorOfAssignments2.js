    ...
    function visitCode(ast, locationMap, path, allAssignments, fileID) {
        ...
        estraverse.traverse(ast, {
            enter: function (node, parent) {
                ...
                let extract = false;
                let cur_node_line_num = node.loc.start.line + "-" + node.loc.end.line;
                // console.log(fileID, cur_node_line_num);
                if (fileID === null) {
                    extract = true;
                } else if (cur_node_line_num === fileID) {
                    extract = true;
                }
                if (extract && node && node.type === "Identifier") {
                ...
                let lhs, rhs;
                // let selectedNodeTypes = ['ExpressionStatement', 'VariableDeclaration', 'VariableDeclarator',
                //     'AssignmentExpression', 'AssignmentPattern'];
                if (extract && node && node.type === "AssignmentExpression") {
                    totalAssignments += 1;
