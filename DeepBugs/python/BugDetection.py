...
def evaluation_real_bugs(predictions, out_file_path, code_pieces, type_of_bug):
    '''
    Predictions is a list where the even indices represent the non-buggy
    where as the odd indices are the buggy ones.
    For the evaluation, we want to check how many of the buggy ones are actually
    classified as buggy.

    - When we provided the seeded bugs then it should consider the even indices to be
    non-buggy and the odd to be buggy. So evaluate on the odd indices i.e., 1, 3, 5 ..
    - When we do not provide the seeded bugs, we actually input real bugs that are present
    at the even indices. The odd indices contain instances of artificially generated bugs. We
    want to check for only the even indices i.e., 0, 2
    ...
    out_data = []
    # print(code_pieces)
    n_c = [m for c in code_pieces for m in c]
    code_pieces = n_c
    for i in range(len(code_pieces)):
        buggy = 1
        if i % 2 == 0:
            buggy = 0
        code_piece = code_pieces[i]
        # ---- For Binary Operand task
        if type_of_bug == 'IncorrectBinaryOperand':
            out_data.append(
                {'left': code_piece.left, 'right': code_piece.right, 'op': code_piece.op, 'src': code_piece.src,
                 'p_buggy': float(predictions[i]), 'buggy': buggy})
        # --- For Wrong Assignment task
        elif type_of_bug == 'IncorrectAssignment':
            out_data.append({'lhs': code_piece.lhs, 'rhs': code_piece.rhs, 'src': code_piece.src,
                             'p_buggy': float(predictions[i]), 'buggy': buggy})

    with open(out_file_path, 'w+') as f:
        print(f'Writing prediction results to {f.name}')
        json.dump(out_data, f)


def prepare_xy_pairs(data_paths, learning_data, provided_seeded_bugs=None):
    xs = []
    ys = []
    code_pieces = []  # keep calls in addition to encoding as x,y pairs (to report detected anomalies)

    if provided_seeded_bugs:
        for code_piece in Util.DataReader(data_paths):
            learning_data.code_to_xy_pairs_given_incorrect_example(code_piece, xs, ys, name_to_vector, type_to_vector,
                                                                   node_type_to_vector,
                                                                   code_pieces)
    else:
        all_binops = list(Util.DataReader(data_paths))
        # all_binops=all_binops[:100]
        for code_piece in tqdm(all_binops, desc='Preparing x,y pairs'):
...
if __name__ == '__main__':
    ...
    semseed_bugs = sys.argv.pop()
    provided_seeded_bugs = True
    if semseed_bugs == '--Default':
        provided_seeded_bugs = False
    epochs = 5
    suffix = f'artificial_wrong_{what}'
    if provided_seeded_bugs:
        suffix = f'semseed_wrong_{what}'
    evaluation_out_file_path = f'benchmarks/evaluated_real_bugs_{suffix}.json'
    if option == "--learn":
    ...
    xs_training, ys_training, _ = prepare_xy_pairs(training_data_paths, learning_data,
                                                   provided_seeded_bugs=provided_seeded_bugs)
    ...
    if option == "--load":
        ...
        history = model.fit(xs_training, ys_training, batch_size=100, epochs=epochs, verbose=1)
    ...
    evaluation_real_bugs(predictions=ys_prediction, out_file_path=evaluation_out_file_path,
                         code_pieces=code_pieces_validation, type_of_bug=what)
    # poss_anomalies = []
    ...
    #     anomaly_score = learning_data.anomaly_score(y_prediction_orig,
    #                                                 y_prediction_changed)  # higher means more likely to be anomaly in current code
    #     normal_score = learning_data.normal_score(y_prediction_orig,
    #                                               y_prediction_changed)  # higher means more likely to be correct in current code
    #     is_anomaly = False
    ...
    #         if isinstance(code_piece, list):
    #             for cp in code_piece:
    #                 message = "Score : " + str(anomaly_score) + " | " + cp.to_message()
